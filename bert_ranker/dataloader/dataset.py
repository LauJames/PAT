...
class MSMARCO_PR_Pair_Dataset(object):
    def __init__(self, tokenizer, random_seed=666):
        ...
        self.dev_triples_path = self.dataset_folder_name + "/sampled_set/triples.dev.pairwise.512000.pkl"
        self.victim_pseudo_same_triples = self.dataset_folder_name + "/pseudo_set/pairwise.pseudo.same.5120000.pkl"
    ...
    def data_generator_pairwise_triple(self, mode='train', epoch_sample_num=None, random_seed=666, batch_size=32,
        ...
        if mode == 'train':
            examples, labels = self._load_from_triples_to_pairwise(sample_num=epoch_sample_num)
        elif mode in ['train_pseudo_same']:
            pkl_path = self.victim_pseudo_same_triples
